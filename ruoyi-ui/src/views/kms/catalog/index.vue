<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="目录名称" prop="title">
        <el-input
          v-model="queryParams.title"
          placeholder="请输入目录名称"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="父目录" prop="parentId">
        <el-input
          v-model="queryParams.parentId"
          placeholder="请输入父目录"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini" @click="handleQuery">搜索</el-button>
        <el-button icon="el-icon-refresh" size="mini" @click="resetQuery">重置</el-button>
      </el-form-item>
    </el-form>

    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          type="primary"
          plain
          icon="el-icon-plus"
          size="mini"
          @click="handleAdd"
          v-hasPermi="['kms:catalog:add']"
        >新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="success"
          plain
          icon="el-icon-edit"
          size="mini"
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['kms:catalog:edit']"
        >修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="danger"
          plain
          icon="el-icon-delete"
          size="mini"
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['kms:catalog:remove']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="warning"
          plain
          icon="el-icon-download"
          size="mini"
          @click="handleExport"
          v-hasPermi="['kms:catalog:export']"
        >导出
        </el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="catalogList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center"/>
      <el-table-column label="主键ID" align="center" prop="id"/>
      <el-table-column label="目录名称" align="center" prop="title"/>
      <el-table-column label="父目录" align="center" prop="parentId"/>
      <el-table-column label="排序号" align="center" prop="sort"/>
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
            icon="el-icon-edit"
            @click="handleUpdate(scope.row)"
            v-hasPermi="['kms:catalog:edit']"
          >修改
          </el-button>
          <el-button
            size="mini"
            type="text"
            icon="el-icon-delete"
            @click="handleDelete(scope.row)"
            v-hasPermi="['kms:catalog:remove']"
          >删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>

    <pagination
      v-show="total>0"
      :total="total"
      :page.sync="queryParams.pageNum"
      :limit.sync="queryParams.pageSize"
      @pagination="getList"
    />

    <!-- 添加或修改知识目录对话框 -->
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="目录名称" prop="title">
          <el-input v-model="form.title" placeholder="请输入目录名称"/>
        </el-form-item>
        <el-form-item label="父目录" prop="parentId">
          <!--<el-input v-model="form.parentId" placeholder="请输入父目录" />-->
          <treeselect v-model="form.parentId" :options="catalogOptions" :normalizer="normalizer"
                      placeholder="选择父目录"/>
        </el-form-item>
        <el-form-item label="关联流程">
          <el-select v-model="form.templateId" placeholder="请选择流程模板">
            <el-option
              v-for="item in templateOptions"
              :key="item.templateId"
              :label="item.templateName"
              :value="item.templateId"
            ></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="排序号" prop="sort">
          <!--<el-input v-model="form.sort" placeholder="请输入排序号" />-->
          <el-input-number v-model="form.sort" :min="1" :max="10" size="small"></el-input-number>
        </el-form-item>
        <el-form-item label="目录描述" prop="description">
          <el-input v-model="form.description" type="textarea" placeholder="请输入内容"/>
        </el-form-item>
        <!--<el-form-item label="目录封面" prop="cover">
          <el-input v-model="form.cover" placeholder="请输入目录封面" />
        </el-form-item>-->
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitForm">确 定</el-button>
        <el-button @click="cancel">取 消</el-button>
      </div>
    </el-dialog>
  </div>
</template>

<script>
import {
  listCatalog,
  listCatalogForTree,
  listCatalogExcludeChild,
  getCatalog,
  delCatalog,
  addCatalog,
  updateCatalog
} from "@/api/kms/catalog";
import {listTemplate} from "@/api/activiti/template"
import Treeselect from "@riophae/vue-treeselect";
import "@riophae/vue-treeselect/dist/vue-treeselect.css";

export default {
  name: "Catalog",
  components: {Treeselect},
  data() {
    return {
      // 遮罩层
      loading: true,
      // 选中数组
      ids: [],
      // 非单个禁用
      single: true,
      // 非多个禁用
      multiple: true,
      // 显示搜索条件
      showSearch: true,
      // 总条数
      total: 0,
      // 知识目录表格数据
      catalogList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
      // 查询参数
      queryParams: {
        pageNum: 1,
        pageSize: 10,
        title: null,
        parentId: null,
        sort: null,
        description: null,
        cover: null
      },
      // 表单参数
      form: {},
      // 表单校验
      rules: {},
      // 目录树选项
      catalogOptions: [],
      templateOptions: []
    };
  },
  created() {
    this.getList();
  },
  methods: {
    /** 查询知识目录列表 */
    getList() {
      this.loading = true;
      listCatalog(this.queryParams).then(response => {
        this.catalogList = response.rows;
        this.total = response.total;
        this.loading = false;
      });
    },
    // 取消按钮
    cancel() {
      this.open = false;
      this.reset();
    },
    // 表单重置
    reset() {
      this.form = {
        id: null,
        title: null,
        parentId: null,
        sort: null,
        description: null,
        cover: null
      };
      this.resetForm("form");
    },
    /** 搜索按钮操作 */
    handleQuery() {
      this.queryParams.pageNum = 1;
      this.getList();
    },
    /** 重置按钮操作 */
    resetQuery() {
      this.resetForm("queryForm");
      this.handleQuery();
    },
    // 多选框选中数据
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.id)
      this.single = selection.length !== 1
      this.multiple = !selection.length
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加知识目录";
      listCatalogForTree().then(response => {
        this.catalogOptions = this.handleTree(response.data, "id");
        /*this.catalogOptions = [];
        const menu = { id: 0, title: '主类目', children: [] };
        menu.children = this.handleTree(response.data, "id");
        this.catalogOptions.push(menu);*/
      });
      listTemplate().then(response => {
        const list = response.rows;
        this.templateOptions = list.map(item => {
          return {
            templateId: item.id,
            templateName: item.name
          }
        })
      })
    },
    /** 转换部门数据结构 */
    normalizer(node) {
      if (node.children && !node.children.length) {
        delete node.children;
      }
      return {
        id: node.id,
        label: node.title,
        children: node.children
      };
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getCatalog(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改知识目录";
      });
      listCatalogExcludeChild(row.id).then(response => {
        this.catalogOptions = this.handleTree(response.data, "id");
      });
      listTemplate().then(response => {
        const list = response.rows;
        this.templateOptions = list.map(item => {
          return {
            templateId: item.id,
            templateName: item.name
          }
        })
      })
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCatalog(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCatalog(this.form).then(response => {
              this.$modal.msgSuccess("新增成功");
              this.open = false;
              this.getList();
            });
          }
        }
      });
    },
    /** 删除按钮操作 */
    handleDelete(row) {
      const ids = row.id || this.ids;
      this.$modal.confirm('是否确认删除知识目录编号为"' + ids + '"的数据项？').then(function () {
        return delCatalog(ids);
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("删除成功");
      }).catch(() => {
      });
    },
    /** 导出按钮操作 */
    handleExport() {
      this.download('kms/catalog/export', {
        ...this.queryParams
      }, `catalog_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
