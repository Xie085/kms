<template>
  <div class="activiti-design" :style="wdStyle" v-loading="loading">
    <breadcrumb-header :breadcrumbList="breadcrumbList" background="#ffffff" exist-back :router-name="routerName">
      <template slot="right">
    ...
    <div class="activiti-overflow">
      <div class="form-model" v-show="first">
        ...
        <el-form ref="base" :model="base" :rules="rules" label-width="80px" class="activiti-design-form">
...
export default {
  ...
  methods: {
    ...
    initData() {
      sysAjax({vm: this, path: {'id': this.id}}, apiSysActiviti.findDetail).then(res => {
        if (res?.message) {
          ...
          let {nodeConfig, flowPermission, directorMaxLevel, workFlowDef} = this.processConfig
